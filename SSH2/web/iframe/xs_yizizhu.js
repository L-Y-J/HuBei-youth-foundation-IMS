    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for (var i=0; i<json.length; i++){
                ...
                var td4 = $("<td>"+json[i].subidizeName+"</td>");
                var td5 = $("<td>"+json[i].subidizeDate+"</td>");
                var td6 = $("<td>"+json[i].connect+"</td>");
                var td7 = $("<td></td>");
                var td8 = $("<input type='checkbox' class='check' />");
                td7.append(td8);
...
function addStudent(){
    ...
    var input6 = $("<input id='input6' type='text' style='width: 80px' />");
    var input7 = $("<input onclick='submitAdd()' type='button' value='确定' class='btn-default' />");
    td1.append(input1);
    td2.append(input2);
    td3.append(input3);
    td4.append(input4);
...
function submitAdd(){
    ...
    var subsidize = $("#input4").val();
    var studentDate = $("#input5").val();
    var connect = $("#input6").val();
...
function UpdateStudent(){
    var id,name,school,subsidize,date,connect;
    ...
    $(td).each(function(){
        switch (i){
            ...
            case 3: subsidize = $(this).children().val();$(this).html(subsidize);break;
            case 4: date = $(this).children().val();$(this).html(date);break;
            case 5: connect = $(this).children().val();$(this).html(connect);break;
            case 6: $(this).html($("<input type='checkbox' class='check' />"));
                break;
        }
        i++;
    });
    ...
    $.ajax({
        ...
        type:'POST',
        data:{'id':id,'name':name,'school':school,'subsidize':subsidize,'date':date,'connect':connect},
        dataType:'text'
    });
}
